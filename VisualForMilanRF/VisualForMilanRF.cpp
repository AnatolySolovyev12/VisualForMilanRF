...
void VisualForMilanRF::test2()
{
 ...
 //  QString savedFile = "abc";
   ...
   //if (!ui.autoSender->isChecked())
    //   savedFile = QFileDialog::getSaveFileName(0, "Save XML", fileName, "*.txt"); // ¬ последнем параметре также можно прописать tr("Xml files (*.xml)"). Ёто будет как приписка с указанием формата. ”добно.
